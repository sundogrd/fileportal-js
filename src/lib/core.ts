...
export default class FilePortal {
    ...
    public addTask(file: any, options: TaskOption): BaseTask {
    ...
    public start(tid: string): BaseTask {
        if (!this.hasStarted) {
    ...
    public stop(tid: string): any {
    // code here
    }

    public stopAll(tid: string): any {
    // code here
    }

    public pause(tid: string) {
        this.taskManager.pause(tid);
        return this;
    }

    public resume(tid: string) {
        this.taskManager.resume(tid);
        return this;
    }

    public cancel(tid: string, message?: string, cb?: (task?: Task) => any) {
    ...
    public completed(cb: FilePortalCompleteCB) {
        this.eventEmitter.on(EFilePortalEvents.COMPLETED, function(ctx, tasks) {
            cb.call(this, tasks);
        }, this);
    }

    public uploaded(cb: FilePortalUploadedCB) {
